...
import type { Dialect, ColumnCompileOptions } from './dialect.js'
...
export class SchemaBuilder {
  ...
  async createTable(tableName: string, callback: (table: TableBlueprint) => void): Promise<void> {
  ...
  async alterTable(tableName: string, callback: (table: TableBlueprint) => void): Promise<void> {
    const blueprint = new TableBlueprint(this.connection, 'alter')
    callback(blueprint)
    const dialect = this.connection.getDialect()

    if (blueprint.droppedColumns.length > 0) {
      const dropSQL = dialect.compileDropColumns(tableName, blueprint.droppedColumns)
      await this.connection.raw(`ALTER TABLE ${dialect.wrapIdentifier(tableName)} ${dropSQL}`)
    }
    ...
    if (blueprint.renamedColumns.length > 0) {
      for (const { from, to } of blueprint.renamedColumns) {
        ...
        await this.connection.raw(`ALTER TABLE ${dialect.wrapIdentifier(tableName)} ${renameSQL}`)
      }
    }
    ...
    if (compiledColumns.length > 0) {
      for (const colSQL of compiledColumns) {
        await this.connection.raw(`ALTER TABLE ${dialect.wrapIdentifier(tableName)} ${colSQL}`)
      }
    }
  }
...
export class TableBlueprint {
  ...
  constructor(connection: DatabaseConnection, mode: BlueprintMode) {
  ...
  foreign(column: string): ForeignKeyDefinition {
    const fk: ForeignKeyDef = { column, references: '', on: '', onDelete: null, onUpdate: null }
...
export class Migrator {
  ...
  private async getNextBatchNumber(): Promise<number> {
    const result = await this.connection.raw('SELECT COALESCE(MAX(batch), 0) + 1 as next_batch FROM migrations')
    return Number(result.rows[0]?.next_batch ?? 1)
  }

  private async getLastBatchNumber(): Promise<number> {
    const result = await this.connection.raw('SELECT COALESCE(MAX(batch), 0) as last_batch FROM migrations')
    return Number(result.rows[0]?.last_batch ?? 0)
  }

  private async getMigrationsByBatch(batch: number): Promise<MigrationStatusRow[]> {
    const result = await this.connection.raw(
