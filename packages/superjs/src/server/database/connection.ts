...
import type { ConnectionConfig, DatabaseDriver, QueryResult, QueryRunner } from './types.js'
import { createDriver, type Driver, type Dialect } from './driver.js'
import { QueryBuilder } from './query.js'

export type { DatabaseDriver }
export type { ConnectionConfig, QueryResult }
...
export class DatabaseConnection implements QueryRunner {
  ...
  async transaction<T>(callback: (trx: DatabaseConnection) => Promise<T>): Promise<T> {
    ...
    return this.driver!.transaction(async (_driver: Driver) => {
      ...
      trxConnection.raw = async (sql: string, bindings?: any[]): Promise<QueryResult> => {
        return this.driver!.raw(sql, bindings)
      }
