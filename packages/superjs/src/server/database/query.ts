...
interface WhereClause {
  type: 'basic' | 'in' | 'notIn' | 'null' | 'notNull' | 'between' | 'notBetween' | 'like' | 'nested'
  column?: string
  operator?: string
  value?: any
  values?: any[]
...
export class QueryBuilder {
  ...
  toSQL(): { sql: string; bindings: any[] } {
    ...
    const wrappedColumns = this.columns
      .map(c => c.includes('(') || c === '*' || c.includes(' as ') || c.includes(' AS ')
        ? c
  ...
  private compileSingleWhere(
    ...
    switch (w.type) {
      ...
      case 'in': {
        const placeholders = w.values!.map((v: any) => {
          bindings.push(v)
  ...
  private compileUpdate(data: Record<string, any>): { sql: string; bindings: any[] } {
    const dialect = this.connection.getDialect()
    const bindings: any[] = []
