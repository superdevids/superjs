...
export class MysqlDialect implements Dialect {
  ...
  private mapType(type: string, options: ColumnCompileOptions): string {
    switch (type) {
      case 'id':
      case 'increments': return 'INT AUTO_INCREMENT PRIMARY KEY'
  ...
  compileTableBlueprint(
    ...
    const parts = [...columns.map(c => this.compileColumn(c)), ...constraints]
    return parts.join(',\n  ')
  }
...
export class SqliteDialect implements Dialect {
  ...
  private mapType(type: string, options: ColumnCompileOptions): string {
    switch (type) {
      ...
      case 'bigIncrements': return 'INTEGER PRIMARY KEY AUTOINCREMENT'
      case 'string': return `VARCHAR(${options.length ?? 255})`
      case 'text': return 'TEXT'
      case 'integer': return 'INTEGER'
      case 'bigInteger': return 'INTEGER'
      case 'tinyInteger': return 'INTEGER'
      case 'smallInteger': return 'INTEGER'
...
export class PostgresqlDialect implements Dialect {
  ...
  private mapType(type: string, options: ColumnCompileOptions): string {
    switch (type) {
      case 'id':
      case 'increments': return 'SERIAL PRIMARY KEY'
